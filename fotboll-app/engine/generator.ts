...
export function generateDefendingCross(level: Level): MatchFreezeQuestion {
  ...
  // Key reference zones
  const def = z.boxDef;
  const pen = z.penaltyBoxDef;
  const six = z.sixYardBoxDef;
  const penaltySpot = z.cornerDefHelpers('right').penaltySpot; // same spot for both sides

  // Ball carrier (opponent winger) on our right flank, deep near byline
  const crosser = { x: z.rightX - 0.02, y: Math.min(0.92, z.defThirdY + 0.18) };

  // Opponent targets: near-post run, far-post run, striker at penalty spot, edge-of-box
  const nearPost = { x: six.x + six.width - 0.02, y: six.y + six.height * 0.2 };
  const farPost = { x: six.x + 0.02, y: six.y + six.height * 0.25 };
  const spot = { x: penaltySpot.x, y: penaltySpot.y };
  const edge = { x: pen.x + pen.width * 0.55, y: pen.y - 0.04 };

  // Home defenders rough positions: RB engages crosser, CBs central, LB far post
  const rb = { x: Math.max(def.x + def.width * 0.85, crosser.x - 0.06), y: crosser.y - 0.02 };
  const cb1 = { x: def.x + def.width * 0.55, y: pen.y + pen.height * 0.35 };
  const cb2 = { x: def.x + def.width * 0.45, y: pen.y + pen.height * 0.45 };
  const lb = { x: def.x + def.width * 0.15, y: pen.y + pen.height * 0.4 };

  // You are center back
  const you = { x: cb2.x, y: cb2.y };

  // Correct zone: central cover between penalty spot and near post (clear first ball)
  const coverRect = {
    x: Math.min(spot.x, nearPost.x) - 0.08,
    y: Math.min(spot.y, nearPost.y) - 0.06,
    width: Math.abs(nearPost.x - spot.x) + 0.16,
    height: Math.abs(nearPost.y - spot.y) + 0.12,
  };

  const players: Array<{ id: string; x: number; y: number; team: 'home' | 'away' }> = [
    // Home (defenders)
    { id: 'you', x: you.x, y: you.y, team: 'home' },
    { id: 'h_rb', x: rb.x, y: rb.y, team: 'home' },
    { id: 'h_cb', x: cb1.x, y: cb1.y, team: 'home' },
    { id: 'h_lb', x: lb.x, y: lb.y, team: 'home' },
    // Away (attackers)
    { id: 'a_winger', x: crosser.x, y: crosser.y, team: 'away' },
    { id: 'a_near', x: nearPost.x, y: nearPost.y, team: 'away' },
    { id: 'a_far', x: farPost.x, y: farPost.y, team: 'away' },
    { id: 'a_9', x: spot.x, y: spot.y, team: 'away' },
    { id: 'a_edge', x: edge.x, y: edge.y, team: 'away' },
  ];
  ...
  return {
    ...
    category: 'forsvar',
    position: 'back',
    question: 'Motståndaren slår inlägg från höger. Du är mittback – var ska du stå?',
    players,
